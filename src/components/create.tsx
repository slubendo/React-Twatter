...
const currentUser = fakeDatabase.getUser('sam')

export default function Create() {
  return (
    <form className="flex flex-wrap border-2">
      <div className="flex justify-start w-1/4 h-10 ">
        <img className="rounded-full overflow-hidden" src={currentUser?.avatar} alt="" />
      </div>
      <div className="w-3/4">
        <p>{currentUser?.username}</p>
        <input type="text" name="content" placeholder='Post a thing...' required></input>
      ...
      <p className="w-2/4">Anyone can reply</p>
      <div className="flex justify-end w-2/4">
        <button className="px-4 py-2 border-2">Post</button>
      </div>
    </form>
  )
}
